...
#include "Participate.h"

frudp_part_t *Participate::frudp_part_find(const frudp_guid_prefix_t *guid_prefix)
{
...
bool Participate::frudp_part_create(void)
{
    if (g_frudp_participant_init_complete)
    {
        printf("woah there partner. freertps currently only allows one participant.\r\n");
        return false;
    }
